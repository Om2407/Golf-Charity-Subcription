...
const { sendWinnerEmail, sendDrawPublishedEmail } = require('../utils/email');
...
router.post('/publish', protect, adminOnly, async (req, res) => {
  try {
    ...
    const allActiveUsers = await User.find({ 'subscription.status': 'active' }, 'name email');
    allActiveUsers.forEach(u => {
      sendDrawPublishedEmail({ email: u.email, name: u.name, month, year, drawnNumbers });
    });

    // Send winner-specific emails (non-blocking)
    const allWinners = [
      ...fiveWinners.map(w => ({ ...w, matchType: 'five_match', prize: fiveMatchPool / (fiveWinners.length || 1) })),
      ...fourWinners.map(w => ({ ...w, matchType: 'four_match', prize: fourMatchPool / (fourWinners.length || 1) })),
      ...threeWinners.map(w => ({ ...w, matchType: 'three_match', prize: threeMatchPool / (threeWinners.length || 1) }))
    ];
    allWinners.forEach(w => {
      sendWinnerEmail({
        name: w.user.name, email: w.user.email,
        matchType: w.matchType, prizeAmount: w.prize,
        drawnNumbers, month, year
      });
    });
