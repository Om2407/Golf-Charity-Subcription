...
router.post('/create-order', protect, async (req, res) => {
  try {
    ...
    await Transaction.create({
      user: req.user._id, type: 'subscription', plan, amount,
      razorpayOrderId: order.id, status: 'created'
    });
    res.json({
      success: true, order,
      key: process.env.RAZORPAY_KEY_ID, amount, plan,
  ...
  try {
    ...
    await Transaction.findOneAndUpdate(
      { razorpayOrderId: razorpay_order_id },
      {
        ...
        status: 'paid', charityAmount, prizePoolAmount,
    ...
    sendSubscriptionEmail({ name: req.user.name, email: req.user.email, plan, endDate });

    res.json({
      success: true,
      message: 'Payment verified! Subscription activated.',
      subscription: { status: 'active', plan, startDate, endDate }
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.post('/donate', protect, async (req, res) => {
  try {
    ...
    if (!amount || amount < 100) {
      return res.status(400).json({ success: false, message: 'Minimum donation is Rs.1' });
    }
    const amountInPaise = amount * 100;
    const options = {
      amount: amountInPaise, currency: 'INR',
      receipt: `don_${Date.now()}`,
      notes: { userId: req.user._id.toString(), charityId, type: 'charity_donation' }
    };
    const order = await razorpay.orders.create(options);
    await Transaction.create({
      user: req.user._id, type: 'charity_donation', amount: amountInPaise,
      razorpayOrderId: order.id, charity: charityId, status: 'created'
    });
    res.json({ success: true, order, key: process.env.RAZORPAY_KEY_ID, amount: amountInPaise });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.get('/history', protect, async (req, res) => {
  try {
    const transactions = await Transaction.find({ user: req.user._id })
      .populate('charity', 'name').sort({ createdAt: -1 });
